    ...
    try:
        return {
            "resultats": {
                k: {
                    "note": v,
                    "valeur": float(indicateurs.get(k.lower().replace(' ', '_'), 0)),
                    "seuils": seuils.get(k.lower().replace(' ', '_'), [0, 0, 0, 0])
...
def generate_pdf(data, company_name, year):
    ...
    try:
            ...
            <div class="section">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td class="note-{{resultat.note}}">{{resultat.note}}</td>
                    </tr>
                    {% endfor %}
                </table>
            </div>
        ...
        template_data = {
            ...
            'conclusion': f"L'analyse des données de {company_name} révèle des opportunités significatives d'amélioration en matière de diversité et d'inclusion. Les actions recommandées, si elles sont mises en œuvre de manière cohérente et suivie, permettront de progresser vers une organisation plus inclusive et équitable.",
